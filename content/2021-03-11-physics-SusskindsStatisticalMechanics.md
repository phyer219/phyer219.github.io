...
- 有一个可变的 chemical potential (Note that $\mu$ is called the chemical
  potential because in general it is different for different kinds of
  molecules.)
...
- chemical potential 对应外加磁场再加一个 shift. chemical potential 就是增加一个
  ...
  a down to up, 它会打开四个 bound, 同时加上外磁场的能量 , energy cost 就是
  $8 J + 2h$ . 故 chemical potential $\mu = 8J + 2h$
